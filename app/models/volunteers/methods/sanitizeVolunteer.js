...
module.exports = (Volunteers, sequelize, DataTypes) => {
	return async (volInfo, user) => {
    		let sanitizedVolunteers = [];

    		for await (let volunteer of volInfo) {

        		if (volunteer) {
          			if (volunteer.roles) {
            				if (volunteer.roles.length >= 1) {
              					volunteer.dateCreated = volunteer.dateCreated || volunteer.firstVolunteered || volunteer.earliest_membership_date;

              					volunteer.full_name = volunteer.first_name + " " + volunteer.last_name;

              					if (!volunteer.contactPreferences) {
                					volunteer.contactPreferences = { donations: true };
              					}
              					...
              					if (volunteer.lastVolunteered) {
                					...
                					} else if (moment(volunteer.lastVolunteered).isBetween(moment().subtract(1, "months"), moment().subtract(2, "weeks"))) {
						...
						if (moment(volunteer.lastCheckin || volunteer.dateCreated).isBefore(moment().subtract(3, "months"))) {
							volunteer.needsToCheckin = "now";
						} else if (moment(volunteer.lastCheckin || volunteer.dateCreated).isBetween(moment().subtract(3, "months"), moment().subtract(2, "months"))) {
							volunteer.needsToCheckin = "soon";
						} else {
							volunteer.needsToCheckin = false;
						}

						volunteer.nextCheckinDue = moment(volunteer.lastCheckin || volunteer.dateCreated).add(3, "months");
						...
						for await (const role of volunteer.roles) {
							if(user.allVolunteerRoles) {
								if(user.allVolunteerRoles[role]) {
									...
									volunteer.formatted_roles += "<li><p><b>" + user.allVolunteerRoles[role].details.title + "</b>";

									if (user.allVolunteerRoles[role].public == 1 && user.allVolunteerRoles[role].removed == 0) {
										volunteer.formatted_roles += " - <a href='https://www.shrubcoop.org/volunteer/view/?roleId=" + role + "'>view role details</a></p>";
									} else {
										volunteer.formatted_roles += "</p>";
									}

									if (user.allVolunteerRoles[role].details.short_description) {
										volunteer.formatted_roles += "<p>" + user.allVolunteerRoles[role].details.short_description + "</p></li>";
									} else {
										volunteer.formatted_roles += "</li>";
									}

									if (user.allWorkingGroupsObj[user.allVolunteerRoles[role].group_id] && !volunteer.working_groups.includes(user.allVolunteerRoles[role].group_id)) {
										volunteer.wg_summary += "<p><b>" + user.allWorkingGroupsObj[user.allVolunteerRoles[role].group_id].name + "</b></p>";

										if (user.allWorkingGroupsObj[user.allVolunteerRoles[role].group_id].welcomeMessage) {
											volunteer.wg_summary += "<div>" + user.allWorkingGroupsObj[user.allVolunteerRoles[role].group_id].welcomeMessage + "</div>";
										}
									}
									...
									try {
										if (user.allWorkingGroupsObj[user.allVolunteerRoles[role].group_id].parent) {
											volunteer.working_groups.push(user.allWorkingGroupsObj[user.allVolunteerRoles[role].group_id].parent);
										}
									} catch (error) {}

									if (volunteer.working_groups.indexOf(user.allVolunteerRoles[role].group_id) == -1) {
										volunteer.old_working_groups.splice(volunteer.working_groups.indexOf(user.allVolunteerRoles[role].group_id), 1);
									}
								}
							}
						}
						...
						let sanitizedVolunteer = {};

						const commonWorkingGroup = Helpers.hasOneInCommon(volunteer.working_groups, user.working_groups);

						const isCoordinator = volunteer.assignedCoordinators.includes(user.id);

						if (user.permissions.volunteers.shiftHistory == true || (user.permissions.volunteers.shiftHistory == "commonWorkingGroup" && commonWorkingGroup) || (user.permissions.volunteers.shiftHistory == "isCoordinator" && isCoordinator)) {
						...
						if (user.permissions.volunteers.conductCheckIn == true || (user.permissions.volunteers.conductCheckIn == "commonWorkingGroup" && commonWorkingGroup) || (user.permissions.volunteers.conductCheckIn == "isCoordinator" && isCoordinator)) {
						...
						if (user.permissions.volunteers.dates == true || (user.permissions.volunteers.dates == "commonWorkingGroup" && commonWorkingGroup) || (user.permissions.volunteers.dates == "isCoordinator" && isCoordinator)) {
						...
						if (user.permissions.volunteers.roles == true || (user.permissions.volunteers.roles == "commonWorkingGroup" && commonWorkingGroup) || (user.permissions.volunteers.roles == "isCoordinator" && isCoordinator)) {
						...
						if (user.permissions.volunteers.assignedCoordinators == true || (user.permissions.volunteers.assignedCoordinators == "commonWorkingGroup" && commonWorkingGroup) || (user.permissions.volunteers.assignedCoordinators == "isCoordinator" && isCoordinator)) {
							sanitizedVolunteer.assignedCoordinators = volunteer.assignedCoordinators;
						}

						if (user.permissions.volunteers.availability == true || (user.permissions.volunteers.availability == "commonWorkingGroup" && commonWorkingGroup) || (user.permissions.volunteers.availability == "isCoordinator" && isCoordinator)) {
							sanitizedVolunteer.availability = volunteer.availability;
						}

						if (user.permissions.volunteers.survey == true || (user.permissions.volunteers.survey == "commonWorkingGroup" && commonWorkingGroup) || (user.permissions.volunteers.survey == "isCoordinator" && isCoordinator)) {
							sanitizedVolunteer.survey = volunteer.survey;
						}

						if (user.permissions.volunteers.emergencyContact == true || (user.permissions.volunteers.emergencyContact == "commonWorkingGroup" && commonWorkingGroup) || (user.permissions.volunteers.emergencyContact == "isCoordinator" && isCoordinator)) {
						...
						if (user.permissions.volunteers.update == true || (user.permissions.volunteers.update == "commonWorkingGroup" && commonWorkingGroup) || (user.permissions.volunteers.update == "isCoordinator" && isCoordinator)) {
						...
						try {
							if (user.permissions.volunteers.manageFoodCollectionLink == true || (user.permissions.volunteers.manageFoodCollectionLink == "commonWorkingGroup" && commonWorkingGroup) || (user.permissions.volunteers.manageFoodCollectionLink == "isCoordinator" && isCoordinator)) {
