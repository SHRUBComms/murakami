    ...
    try {
      if (members[collection.member_id]) {
        collection.collectedBy =
          members[collection.member_id].first_name + " " + members[collection.member_id].last_name;
      } else {
      ...
      for await (const organisation_id of collection.destination_organisations) {
        ...
        if (
          collection.destination_organisations.indexOf(organisation_id) !=
          collection.destination_organisations.length - 1
        ) {
          collection.droppedOffTo += ", ";
        }
      }
