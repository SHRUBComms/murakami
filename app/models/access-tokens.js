...
AccessTokens.add = function(page, callback) {
  var query =
    "INSERT INTO access_tokens (token, page, date, used) VALUES (?,?,?,0)";
...
AccessTokens.get = function(token, page, callback) {
  var query =
    "SELECT * FROM access_tokens WHERE page = ? AND token = ? AND date >= DATE_SUB(NOW(), INTERVAL 1 DAY) AND used = 0";
  var dt = new Date();
...
AccessTokens.getById = function(token, callback) {
  var query =
    "SELECT * FROM access_tokens WHERE token = ? AND date >= DATE_SUB(NOW(), INTERVAL 1 DAY) AND used = 0";
  var dt = new Date();
