	...
	return async (users, loggedInUserObj) => {
		...
		for await (const user of users) {
			...
			if (user.working_groups) {
          		...
          		const commonWorkingGroup = Helpers.hasOneInCommon(loggedInUser.working_groups, user.working_groups);

          		try {
            			if (loggedInUser.permissions.users.name == true || (loggedInUser.permissions.users.name == "commonWorkingGroup" && commonWorkingGroup) || isUser) {
          		...
          		try {
            			if (loggedInUser.permissions.users.email == true || (loggedInUser.permissions.users.email == "commonWorkingGroup" && commonWorkingGroup) || isUser) {
              				sanitizedUser.email = user.email;
            			}
          		} catch (error) {}

          		try {
            			if (loggedInUser.permissions.users.username == true || (loggedInUser.permissions.users.username == "commonWorkingGroup" && commonWorkingGroup) || isUser) {
              				sanitizedUser.username = user.username;
            			}
          		} catch (error) {}

          		try {
            			if (loggedInUser.permissions.users.workingGroups == true || (loggedInUser.permissions.users.workingGroups == "commonWorkingGroup" && commonWorkingGroup) || isUser) {
              				sanitizedUser.working_groups = user.working_groups;
            			}
          		} catch (error) {}

          		try {
            			if (loggedInUser.permissions.users.update == true || (loggedInUser.permissions.users.update == "commonWorkingGroup" && commonWorkingGroup) || isUser) {
              				sanitizedUser.canUpdate = true;
            			}
          		} catch (error) {}

          		try {
            			if (loggedInUser.permissions.users.deactivate == true || (loggedInUser.permissions.users.deactivate == "commonWorkingGroup" && commonWorkingGroup) || isUser) {
