...
MailchimpAPI.subscribeToNewsletter = async (newsletterId, newsletterSecret, member) => {
  try {
    ...
    const response = await newsletter.put(
      "/lists/" + process.env.SHRUB_MAILCHIMP_NEWSLETTER_LIST_ID + "/members/" + md5(member.email),
      subscribeBody
    );

    subscribeBody.marketing_permissions = [
      {
        marketing_permission_id: response.marketing_permissions[0].marketing_permission_id,
...
MailchimpAPI.unsubscribeFromNewsletter = async (newsletterId, newsletterSecret, member) => {
  try {
    ...
    const result = await newsletter.delete(
      "/lists/" + process.env.SHRUB_MAILCHIMP_NEWSLETTER_LIST_ID + "/members/" + md5(member.email)
    );
    return true;
  } catch (error) {
...
MailchimpAPI.isSubscribedToNewsletter = async (newsletterId, newsletterSecret, email) => {
  try {
    ...
    const response = await newsletter.get({
      path: "/lists/" + newsletterId + "/members/" + md5(email),
    });
    return true;
  } catch (error) {
    return false;
