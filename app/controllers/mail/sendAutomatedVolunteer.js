...
const dynamicVariablesAvailable = require(
  rootDir + "/app/controllers/mail/dynamicVariables.config"
);
...
module.exports = (Mail, nodemailer, htmlToText) => {
  return async (mail_id, member_id) => {
    const template = await MailTemplates.getById(mail_id);
    if (!template) {
