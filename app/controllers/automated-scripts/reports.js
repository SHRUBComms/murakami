  ...
  onTick: async () => {
    try {
      ...
      for await (const member of members) {
        ...
        if (member.earliest_membership_date == member.current_init_membership && moment(member.current_init_membership).startOf("month").isSame(startOfMonth)) {
        ...
        } else if (!moment(member.earliest_membership_date).isSame(member.current_init_membership) && moment(member.current_init_membership).startOf("month").isSame(startOfMonth)) {
      ...
      const shifts = await VolunteerHours.getAllApprovedBetweenTwoDates(moment(startOfMonth).toDate(), moment(startOfMonth).endOf("month").toDate());
