...
const failedTransactions = new CronJob({
  ...
  onTick: async () => {
    ...
    const fxTransactions = {}; // SumUp transactions by ID.
    /* 
	...
	*/
    try {
      let transactions = await Transactions.getAllBetweenTwoDates(
      ...
      for await (const transaction of transactions) {
        ...
        const sumupTransactions = await Helpers.SumUpGetTransactionsBetweenTwoDates(
          startTimestamp,
          endTimestamp,
          accessToken
        );
        ...
        for await (const fxTransaction of sumupTransactions.items) {
          ...
          const fxInUse = await Transactions.findOne({
            where: { summary: { sumupId: fxTransaction.transaction_code } },
          });
          ...
          const updatedSummary = transaction.summary;
