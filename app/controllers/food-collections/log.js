...
const LogFoodCollection = async (req, res, foodCollectionKey) => {
  try {
    ...
    const availableOrganisations = lodash.spread(lodash.union)([
      lodash.clone(foodCollectionKey.organisations),
      Object.keys(defaultOrganisations),
    ]);

    if (
      !Helpers.allBelongTo(collectionOrganisation, availableOrganisations) ||
      !Helpers.allBelongTo(destinationOrganisations, availableOrganisations)
    ) {
