...
const validateRole = async (loggedInUser, submittedForm, validSelections) => {
  try {
    ...
    if (
      Helpers.allBelongTo(submittedForm.locations, Object.keys(validSelections.locations)) == false
    ) {
      throw "Please make sure you have selected valid locations";
    }

    if (
      Helpers.allBelongTo(submittedForm.activities, Object.keys(validSelections.activities)) ==
      false
    ) {
