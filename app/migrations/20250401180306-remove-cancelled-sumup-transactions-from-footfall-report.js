    ...
    return queryInterface.sequelize.query(`
      ...
      SELECT      tr.\`date\` AS 'transaction_date'
          ...
          ,       CASE
                      ...
                      WHEN HOUR(tr.\`date\`) = 18 THEN '6pm-7pm'
                      WHEN HOUR(tr.\`date\`) = 19 THEN '7pm-8pm'
                      WHEN HOUR(tr.\`date\`) BETWEEN 20 AND 23 THEN '8pm Onwards'
                  END AS 'transaction_hour'
      FROM        murakami.transactions tr
      INNER JOIN  murakami.tills ti
      ON          ti.till_id = tr.till_id
      WHERE       JSON_EXTRACT(tr.summary, '$.sumupId') IS NOT NULL
      OR          IFNULL(JSON_EXTRACT(tr.summary, '$.paymentMethod'), "") <> '"card"';
    `);
  },
