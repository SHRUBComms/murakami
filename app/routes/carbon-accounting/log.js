    ...
    try {
      ...
      for await (const entry of transaction) {
        ...
        if (!formattedTransaction.trans_object[entry.id]) {
          ...
          formattedTransaction.trans_object[entry.id] =
            +entry.weight + +formattedTransaction.trans_object[entry.id];
        }
      ...
      res.send({
        status: "ok",
        msg: `Weight logged! ${Math.abs(totalCarbon * 1e-3).toFixed(2)}kg of carbon saved`,
      });
    } catch (error) {
      console.log(error);
