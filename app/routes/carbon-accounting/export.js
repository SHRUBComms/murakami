      ...
      if (!invalidRequest) {
        ...
        for await (const transaction of rawCarbon) {
          ...
          for await (const carbonCategoryId of Object.keys(transaction.trans_object)) {
            ...
            formattedData[transaction.method][carbonCategoryId].saved +=
              +transaction.trans_object[carbonCategoryId] *
              +carbonCategories[carbonCategoryId].factors[transaction.method];
        ...
        for await (const disposalMethod of Object.keys(formattedData)) {
          if (formattedData[disposalMethod].raw) {
            formattedData[disposalMethod].raw = (
              formattedData[disposalMethod].raw * unit.factor
            ).toFixed(4);
            formattedData[disposalMethod].saved = (
              formattedData[disposalMethod].saved * unit.factor
            ).toFixed(4);
          } else {
            for await (const carbonId of Object.keys(formattedData[disposalMethod])) {
              if (formattedData[disposalMethod][carbonId]) {
                formattedData[disposalMethod][carbonId].raw = (
                  formattedData[disposalMethod][carbonId].raw * unit.factor
                ).toFixed(4);
                formattedData[disposalMethod][carbonId].saved = (
                  formattedData[disposalMethod][carbonId].saved * unit.factor
                ).toFixed(4);
              }
            }
          }
