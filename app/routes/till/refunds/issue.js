  ...
  async (req, res) => {
    try {
      ...
      const tillBalance =
        ...
        (Number(totalTakings) - Number(totalRefunds) - Number(totalReimbursements));
      ...
      if (moment(transaction.date).endOf("day").isBefore(moment().subtract(refundPeriod, "days"))) {
        // come back
      ...
      if (action == "lookup") {
        ...
        if (transaction.summary.sumupId) {
          ...
          res.send({
            status: "ok",
            msg: `Refund successfully recorded - please give the customer £${Number(amount).toFixed(2)}`,
          });
          response.status = "ok";
        }
      }
