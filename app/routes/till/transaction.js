    ...
    try {
      ...
      if (
        !(
          ...
          (req.user.permissions.tills.processTransaction == "commonWorkingGroup" &&
            req.user.working_groups.includes(till.group_id))
        )
      ) {
        throw "You don't have permission to process transactions on this till";
      ...
      // Discounts
      const discountInfo = {}; // Contains IDs of items that have a members' discount
      ...
      for await (const item of transaction) {
        ...
        if (item.id == "giftcard") {
          ...
          const category = lodash.clone(categories[id]);
          ...
          if (category.discount == 1) {
            ...
            const group_id = category.group_id || till.group_id;
            ...
            if (weight > 0 && carbonCategories[item.carbon_id]) {
              ...
              if (carbonTransaction[group_id][item.carbon_id]) {
                carbonTransaction[item.carbon_id] =
                  Number(carbonTransaction[item.carbon_id]) + Number(weight * quantity);
              } else {
      ...
      const member = await Members.getById(member_id, {
        permissions: { members: { name: true, balance: true, membershipDates: true } },
      });
        ...
        if (payWithTokens == true) {
          if (totalMoney == 0) {
            if (member.balance >= totalTokens) {
              ...
              const difference = totalTokens - member.balance;
