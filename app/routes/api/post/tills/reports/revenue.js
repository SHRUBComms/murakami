...
router.post("/", Auth.verifyByKey("tillRevenue"), async (req, res) => {
  try {
    ...
    for (const transaction of transactions) {
      ...
      const monthKey = moment(transaction.date).startOf("month").format("YYYY-MM-DD");
      ...
      if (transaction.summary.bill[0].item_id == "yoyoCup") {
        ...
        response.summary[monthKey].revenue.breakdown.cash += -Number(
          transaction.summary.totals.cash
        );
      }
      ...
      for await (const item of transaction.summary.bill) {
        ...
        if (group_id !== null) {
          if (transaction.summary.bill[0].item_id == "yoyoCup") {
            ...
            response.summary[monthKey].byGroup[tillsObj[transaction.till_id].group_id].total +=
              -Number(transaction.summary.totals.cash);
            response.summary[monthKey].byGroup[
              tillsObj[transaction.till_id].group_id
            ].breakdown.cash += -Number(transaction.summary.totals.cash);
            continue;
          }
          ...
          let itemValue =
            parseFloat(
              (parseFloat(item.value) || parseFloat(item.tokens)) * (parseInt(item.quantity) || 1)
            ) || 0;
          ...
          if (
            response.summary[monthKey].revenue.breakdown[transaction.summary.paymentMethod] !==
            undefined
          ) {
            response.summary[monthKey].revenue.breakdown[transaction.summary.paymentMethod] +=
              Number(itemValue);
          ...
          if (
            response.summary[monthKey].byGroup[group_id].breakdown[
    ...
    const earliestTransactionRecord = moment("2018-11-01");

    const monthDifference = Array.from(
      Array(Math.floor(moment().diff(moment(earliestTransactionRecord), "months"))).keys()
    );

    for await (const i of monthDifference) {
      const monthKey = moment(moment(earliestTransactionRecord).add(i, "months"))
        .startOf("month")
        .format("YYYY-MM-DD");
