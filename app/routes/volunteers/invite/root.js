...
router.get("/", Auth.isLoggedIn, Auth.canAccessPage("volunteers", "invite"), async (req, res) => {
    ...
    try {
        ...
        const defaultFoodCollectorRole = await VolunteerRoles.getRoleById(defaultFoodCollectorRoleId.data.role_id);
...
router.post("/", Auth.isLoggedIn, Auth.canAccessPage("volunteers", "invite"), async (req, res) => {
    ...
    try {
        ...
        if (member && !member.volunteer_id) {
            ...
            let volunteer = await Volunteers.getVolunteerById(member.volunteer_id, {
                permissions: {
                    volunteers: {
                        roles: true,
...
router.get("/:token", Auth.isNotLoggedIn, Auth.hasValidToken("add-volunteer"), async (req, res) => {
  try {
    ...
    const { coordinatorsObj } = await Users.getCoordinators(req.user);
    const { rolesObj, skills, contactMethods, volunteerAgreement, ourVision, saferSpacesPolicy, membershipBenefits, privacyNotice } = await Volunteers.getSignUpInfo();
