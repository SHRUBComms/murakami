...
router.get("/", Auth.isLoggedIn, Auth.canAccessPage("volunteerHours", "export"), async (req, res) => {
    	try {
		let group_id = null;
	    	if (req.user.permissions.volunteerHours.export == true || (req.user.permissions.volunteerHours.export == "commonWorkingGroup" && req.user.working_groups.includes(req.query.group_id))) {
		...
		if(group_id) {
			...
			shifts = await VolunteerHours.getHoursBetweenTwoDatesByWorkingGroup(group_id, req.query.startDate, req.query.endDate, membersObj);
		}
