    ...
    VolunteerRoles.getRoleById(req.params.role_id, function(err, roleExists) {
      if (roleExists) {
        if (
          req.user.permissions.volunteerRoles.update == true ||
          (req.user.permissions.volunteerRoles.update == "commonWorkingGroup" &&
            req.user.working_groups.includes(roleExists.group_id))
        ) {
          VolunteerRoles.getRoleSignUpInfo(function(
            allLocations,
            allActivities,
            ...
            if (errors[0]) {
              ...
              if (req.user.permissions.volunteerRoles.update == true) {
                validWorkingGroups = req.user.allWorkingGroupsFlat;
              } else if (
                req.user.permissions.volunteerRoles.update ==
                "commonWorkingGroup"
              ) {
              ...
              if (role.working_group) {
                if (!validWorkingGroups.includes(role.working_group)) {
                  var error = {
                    param: "working_group",
