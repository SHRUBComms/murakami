...
router.get("/", Auth.isLoggedIn, Auth.canAccessPage("settings", "staticContent"), async (req, res) => {
  const staticContent = await Settings.getStaticContent();
  res.redirect(process.env.PUBLIC_ADDRESS + "/settings/static-content/" + Object.keys(staticContent.texts)[0]);
});

router.get("/:content_id", Auth.isLoggedIn, Auth.canAccessPage("settings", "staticContent"), async (req, res) => {
  try {
    const staticContent = await Settings.getStaticContent();
...
router.post("/:content_id", Auth.isLoggedIn, Auth.canAccessPage("settings", "staticContent"), async (req, res) => {
  try {
    const staticContent = await Settings.getStaticContent();
    let content;
