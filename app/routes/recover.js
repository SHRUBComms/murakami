...
router.post("/", async (req, res) => {
	...
	try {
		...
		const resetToken = await PasswordReset.addPasswordReset(user.id, req.headers["x-forwarded-for"] || req.connection.remoteAddress);
...
router.post("/:resetToken", async (req, res) => {
	...
	try {
		...
		const user = await Users.getById(passwordReset.user_id, { permissions: { users: { name: true } } });
