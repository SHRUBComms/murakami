...
router.post("/", Auth.isLoggedIn, Auth.canAccessPage("users", "add"), async (req, res) => {
  ...
  try {
    ...
    activationToken = await AccessTokens.createInvite(expirationTimestamp, {
      action: "add-user",
    ...
    if (activationToken) {
      error =
        "Something went wrong sending the email! Please send the activation link manually: " +
        activationLink;
    }
...
router.get("/:token", Auth.isNotLoggedIn, async (req, res) => {
  try {
    ...
    const user = await Users.getById(invite.details.user_id, {
...
router.post("/:token", Auth.isNotLoggedIn, async (req, res) => {
  ...
  try {
    ...
    const userInvitedBy = await Users.getById(invite.details.invitedBy, {
