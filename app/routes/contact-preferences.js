...
router.get("/:member_id", async (req, res) => {
	try {
		const member = await Members.getById(req.params.member_id, { permissions: { members: { name: true, contactDetails: true } } });
		...
		if(!member) {
        	...
        	if (member.contactPreferences) {
          		member.contactPreferences.newsletters = { shrub: null };
        	} else {
          		member.contactPreferences = { newsletters: { shrub: null } };
        	}

        	const isSubscribed = await MailchimpAPI.isSubscribedToNewsletter(process.env.SHRUB_MAILCHIMP_NEWSLETTER_LIST_ID, process.env.SHRUB_MAILCHIMP_SECRET_API_KEY, member.email);
        	if (isSubscribed) {
		...
		if (req.body.newsletters) {
			if(req.body.newsletters.shrub) {
				...
				if (req.body.newsletters.shrub.unsubscribe) {
					...
					await MailchimpAPI.unsubscribeFromNewsletter(process.env.SHRUB_MAILCHIMP_NEWSLETTER_LIST_ID, process.env.SHRUB_MAILCHIMP_SECRET_API_KEY, member);
				}
			}
		}
