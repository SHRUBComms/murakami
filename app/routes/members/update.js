      ...
      if (
        !(
          ...
          (req.user.permissions.members.view == "commonWorkingGroup" &&
            Helpers.hasOneInCommon(member.working_groups, req.user.working_groups))
        )
      ) {
    ...
    if (["staff", "admin"].includes(req.user.class)) {
      if (
        !(
          ...
          ["lifetime", "staff", "trustee", "none"].includes(updatedMember.membership_type)
        )
      ) {
      ...
      if (
        updatedMember.free == "free" ||
        ["lifetime", "staff", "trustee"].includes(updatedMember.membership_type)
      ) {
      ...
      await Validators.number(
        { name: "tokens", indefiniteArticle: "a number of", value: updatedMember.balance },
