...
router.get("/:member_id", Auth.isLoggedIn, Auth.canAccessPage("volunteers", "add"), async (req, res) => {
  try {
    const member = await Members.getById(req.params.member_id, { permissions: { members: { name: true, membershipDates: true, contactDetails: true, workingGroups: true } } });
    if (!member) {
    ...
    const { skills, contactMethods, rolesByGroup, volunteerAgreement } = await Volunteers.getSignUpInfo();
...
router.post("/:member_id", Auth.canAccessPage("volunteers", "add"), async (req, res) => {
  ...
  const { skills, contactMethods, rolesByGroup, rolesObj, volunteerAgreement } = await Volunteers.getSignUpInfo();
  let member;
  ...
  try {
    ...
    member = await Members.getById(req.params.member_id, { permissions: { members: { name: true, membershipDates: true, contactDetails: true, workingGroups: true } } });
    if (!member) {
      throw "Member not found";
    }
